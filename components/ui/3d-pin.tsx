        ...
        <div
          ...
          className={`absolute left-1/2 p-4 top-1/2 flex justify-start items-start rounded-2xl shadow-[0_8px_16px_rgb(0_0_0/0.4)] border ${
            isDarkMode
              ...
              : "group-hover/pin:border-black/[0.2]"
          } transition duration-700 overflow-hidden`}
        >
          <div className={cn("relative z-50", className)}>{children}</div>
        </div>
      </div>
      <PinPerspective title={title == 'Pong Game' ? 'Alien Pong' : title}/>
    </Link>
  );
};
    ...
    <motion.div className="pointer-events-none w-96 h-80 flex items-center justify-center opacity-0 group-hover/pin:opacity-100 z-[60] transition duration-500">
      <div className="w-full h-full -mt-7 flex-none inset-0">
        <div className="absolute top-0 inset-x-0 flex justify-center">
          <span
            className={`relative flex space-x-2 items-center z-10 rounded-full py-0.5 px-4 ring-1 ${
